namespace Wacton.Japangolin.Domain
{
    ...
    public class KanaBase : Enumeration
    {
        ...
        private readonly Dictionary<KanaSyllabary, Dictionary<KanaType, char>> dict; 

        public KanaBase(string friendlyString, char hiraganaSeion, char katakanaSeion)
            : base(counter, friendlyString)
        {
            counter++;
            this.dict = new Dictionary<KanaSyllabary, Dictionary<KanaType, char>>();
            this.dict.Add(KanaSyllabary.Hiragana, new Dictionary<KanaType, char>());
            this.dict.Add(KanaSyllabary.Katakana, new Dictionary<KanaType, char>());

            this.dict[KanaSyllabary.Hiragana].Add(KanaType.Seion, hiraganaSeion);
            this.dict[KanaSyllabary.Katakana].Add(KanaType.Seion, katakanaSeion);
        }
        ...
        {
            this.dict[KanaSyllabary.Hiragana].Add(KanaType.Dakuon, hiraganaDakuon);
            this.dict[KanaSyllabary.Katakana].Add(KanaType.Dakuon, katakanaDakuon);
        }
        ...
        {
            this.dict[KanaSyllabary.Hiragana].Add(KanaType.Handakuon, hiraganaHandakuon);
            this.dict[KanaSyllabary.Katakana].Add(KanaType.Handakuon, katakanaHandakuon);
        }

        public char GetCharacter(KanaSyllabary kanaSyllabary, KanaType kanaType)
        {
        ...
        public override string ToString()
        {
            ...
            foreach (var hiragana in this.dict[KanaSyllabary.Hiragana])
            {
            ...
            foreach (var katakana in this.dict[KanaSyllabary.Katakana])
            {
                stringbuilder.Append(katakana.Value);
            }

            return string.Format("{0} ({1})", this.DisplayName, stringbuilder);
        }
    }
}
